...
component:
	...
	@cat ./src/components/snippet/index.tsx | sed 's?SnippetComponent?'$(name)'?' | sed 's?some-classname?'$(name)'?' | sed 's?ISnippetComponentProp?'I$(name)Prop'?' | sed 's?import "./index.scss"?import "./$(name).module.scss"' > ./src/components/$(role)/$(name)/$(name).tsx
