  ...
  return (
    <div className={classes.main_container}>
      ...
      <Text size={'s'} view={'secondary'} weight={'light'}
            className={classes.version}>{t('unauthorized.installing.version')} {metadata.versions[currentVersion].version} {t('unauthorized.installing.from')} {dayjs(metadata.versions[currentVersion].date).format('DD.MM.YYYY')}</Text>
      <Text size={'s'} view={'secondary'} weight={'light'}
            className={classes.os}>{t('unauthorized.installing.forEach')} {osMatcher[osInfo.detectedOS]} x{osInfo.detectedArch}</Text>
      <div className={classes.other_versions}>
        <Text size={'s'} view={'secondary'} weight={'light'}>{t('unauthorized.installing.other')}</Text>
        <div className={classes.other_version_body}>
              ...
              return (
                <div key={e.os} className={classes.other_version}>
                  ...
                  <a title={e.md5} href={`${axiosConfig.baseUrl}${e.path}`}>
                    <Text cursor={'pointer'} size={'s'} view={'linkMinor'}>{t('unauthorized.installing.forEach')} {e.os} x{e.arch}</Text>
                  </a>
                </div>
              )
            })
          ...
          {currentVersion < metadata.versions.length - 1
            ? <div className={classes.other_version}>
              ...
              }} size={'s'} view={'linkMinor'}>{t('unauthorized.installing.prevVersion')}</Text>
            </div>
            : <></>}
          {currentVersion > 0
            ? <div className={classes.other_version}>
              ...
              <Text cursor={'pointer'} onClick={() => {
                setCurrentVersion(currentVersion - 1)
              }} size={'s'} view={'linkMinor'}>{t('unauthorized.installing.nextVersion')}</Text>
            </div>
            : <></>}
        </div>
      </div>
